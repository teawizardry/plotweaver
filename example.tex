...
Icons are also provided for the metals (both allomantic and feruchemical) in Mistborn and the radiant orders (along with their corresponding surges) in Stormlight Archive. Examples can be seen in Table~\ref{tab:icons}. The matching set of icons will be loaded with the chosen color scheme, but you can also manually load each icon package.

\begin{figure}[h]
\captionsetup[subfigure]{labelformat=empty}
\captionsetup{labelformat=empty}
\centering
\caption{Samples of the Mistborn and Stormlight icons.}
...
\hfil
\begin{subfigure}{0.45\textwidth}
\caption{Stormlight Symbols}
\begin{plotable}{cccl}
    \Block[c]{1-2}{Surges} & & \Block[c]{1-2}{Radiant Order} & \\
    \adhesion & \gravitation & \windrunner & Windrunners\\
    \gravitation & \division & \skybreaker & Skybreakers\\
    \division & \abrasion & \dustbringer & Dustbringers\\
    \abrasion & \progression & \edgedancer & Edgedancers\\
    \progression & \illumination & \truthwatcher & Truthwatchers\\
    \illumination & \transformation & \lightweaver & Lightweavers\\
    \transformation & \transportation & \elsecaller & Elsecallers\\
    \transportation & \cohesion & \willshaper & Willshapers\\
    \cohesion & \tension & \stoneward & Stonewards\\
    \tension & \adhesion & \bondsmith & Bondsmiths\\
\end{plotable}
\end{subfigure}
\label{tab:icons}
\end{figure}
...
\subsection{Lists}
Lists can be made using the \texttt{itemize} environment. They will automatically use the matching color scheme.
\begin{itemize}[align=left]
    \item Item 1
    \item Item 2
    \item[\textcolor{cscolor}{\textbf{Hanging Subheading:}}] You can also use the \texttt{itemize} environment to create short subheadings with a hanging indent. See the \texttt{enumitem} package documentation for more details.
\end{itemize}

\subsection{Statblocks}
Statblocks can be made using the \texttt{\textbackslash MakeStatBlock} command. Similar to the character sheet command, you pass in the attributes, features, and actions for the statblock as key-value pairs. This command currently does not calculate derived attributes, as those tend to not follow the derivation formulae as strictly. You can also change the number of columns in the statblock. Thanks to \texttt{cosmicducks} for contributing this command!
...
\section{A Note on Compilation Times}
For basic document, the compilation time with this document class should be fairly low. I have tried to keep all of the icon definitions separated off into their own setting specific packages to help with compilation times. The character sheet command is in its own separate package for that same reason. Slower compilation times are expected for documents that make heavy use of all of the various design elements provided. I have experimented with \texttt{memoize}, but it has compatibility issues with \texttt{tcolorbox} and requires additional setup on the user end. I have also experimented with \texttt{tikzexternalize}, but found its compilation times to be slower than without externalization.
